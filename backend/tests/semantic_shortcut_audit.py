...
PATTERNS = (
    ...
    ShortcutPattern(
        ...
        classification="legacy_disabled_must_not_enable",
        rationale="Disabled semantic router uses hardcoded examples/hints and must pass the same guard before enablement.",
    ),
    ShortcutPattern(
        id="hybrid_router_prompt_rules",
        path_glob="backend/routing/hybrid_router.py",
        regex=r"_PROVIDER_HINTS|Decision rules:",
        classification="legacy_disabled_must_not_enable",
        rationale="Disabled hybrid router contains explicit semantic provider rules.",
    ),
    ShortcutPattern(
